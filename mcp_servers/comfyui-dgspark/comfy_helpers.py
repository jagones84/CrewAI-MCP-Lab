...
def _get_object_info(timeout: int = 30) -> dict:
    """Fetch the cached ComfyUI ``/object_info`` payload.

    Returns a dict mapping class_type -> metadata. Raises ``RuntimeError`` if
    the endpoint is unreachable so callers can fail fast with a clear error
    rather than waiting 10 minutes on a dead ComfyUI.
    """
    try:
        with urllib.request.urlopen(
            f"http://{COMFYUI_SERVER_ADDRESS}/object_info", timeout=timeout
        ) as response:
            return json.loads(response.read())
    except (urllib.error.URLError, OSError, TimeoutError) as exc:
        raise RuntimeError(
            f"Cannot reach ComfyUI /object_info at {COMFYUI_SERVER_ADDRESS}: {exc}"
        ) from exc


def _choices_for(info: dict, class_type: str, input_name: str) -> list[str]:
    """Return the list of available values for a given node input.

    ComfyUI's ``/object_info`` nests the available choices inside a
    ``COMBO`` descriptor; this helper unwraps it.
    """
    try:
        required = (
            info.get(class_type, {}).get("input", {}).get("required", {})
        )
        descriptor = required.get(input_name)
        if not descriptor:
            return []
        if isinstance(descriptor, list) and descriptor:
            inner = descriptor[0]
            if isinstance(inner, list):
                return [str(c) for c in inner]
            if isinstance(inner, dict) and "options" in inner:
                return [str(c) for c in inner["options"]]
    except (AttributeError, TypeError, IndexError):
        return []
    return []


def validate_workflow_models(workflow: dict) -> list[str]:
    """Pre-flight check: every model referenced by the workflow must be installed.

    Iterates the workflow looking for known loader nodes
    (``UNETLoader``, ``CheckpointLoaderSimple``, ``CLIPLoader``, ``VAELoader``,
    ``UpscaleModelLoader``) and verifies the configured checkpoint path
    exists in the ComfyUI ``/object_info`` payload. Returns a list of
    human-readable errors (empty when the workflow is good).

    Failing fast on a missing checkpoint is the difference between
    ``ComfyUI API Error (400): value_not_in_list`` and a clear,
    actionable ``Missing model on ComfyUI: qwen_image_fp8_e4m3fn.safetensors``.
    """
    info = None
    errors: list[str] = []
    loader_specs = [
        ("UNETLoader", "unet_name", "inputs", "unet_name"),
        ("CheckpointLoaderSimple", "ckpt_name", "inputs", "ckpt_name"),
        ("CLIPLoader", "clip_name", "inputs", "clip_name"),
        ("VAELoader", "vae_name", "inputs", "vae_name"),
        ("UpscaleModelLoader", "model_name", "inputs", "model_name"),
    ]
    for node_id, node in workflow.items():
        if not isinstance(node, dict):
            continue
        class_type = node.get("class_type")
        for loader, key, _container, _input_key in loader_specs:
            if class_type != loader:
                continue
            wanted = (node.get("inputs") or {}).get(key)
            if not wanted:
                continue
            if info is None:
                try:
                    ...
                    return errors
            available = _choices_for(info, loader, key)
            if available and wanted not in available:
                errors.append(
                    f"Node {node_id} ({loader}): {key}={wanted!r} not installed. "
