#!/usr/bin/env python3
"""
ComfyUI background removal helper.
...
def wait_for_completion(max_wait=600, poll_seconds=5):
    ...
    print("Waiting for background removal to complete...")
    waited = 0
...
def build_remove_bg_workflow(uploaded_name, prefix):
    """Return a known-good background removal workflow using LayerStyle."""
    return {
        "1": {
...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: remove_bg_and_send.py /absolute/source/image [output_name]",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    prefix = f"openclaw_removebg_{int(time.time())}"
    ...
    print("Submitting background removal request...")
    workflow = build_remove_bg_workflow(uploaded_name, prefix)
    result = comfyui_json_request(
        "/prompt",
        {
            "prompt": workflow,
            "client_id": f"openclaw-removebg-{uuid.uuid4().hex}",
        },
    )
    ...
    if not wait_for_completion():
        print("Timeout waiting for background removal", file=sys.stderr)
        sys.exit(1)
    ...
    if not source_output_path or not os.path.exists(source_output_path):
        print("Could not locate background-removed image", file=sys.stderr)
        sys.exit(1)
