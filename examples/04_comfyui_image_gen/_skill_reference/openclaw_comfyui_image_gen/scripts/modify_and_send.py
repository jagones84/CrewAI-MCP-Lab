#!/usr/bin/env python3
"""
ComfyUI image modification helper.
...
DEFAULT_CHECKPOINT = "perfectdeliberate_v60.safetensors"
DEFAULT_NEGATIVE = "lowres, bad anatomy, blurry, watermark, text, deformed, ugly, cartoon"
...
def wait_for_completion(max_wait=600, poll_seconds=5):
    ...
    print("Waiting for image modification to complete...")
    waited = 0
...
def build_modify_workflow(uploaded_name, prompt, denoise_strength, prefix):
    """Return a known-good img2img workflow based on the uncensored checkpoint."""
    return {
        "1": {
...
def resolve_image(prompt_id, before_files, started_at):
    """Resolve source image path for the current run."""
...
def main():
    if len(sys.argv) < 3:
        print(
            "Usage: modify_and_send.py /absolute/source/image 'prompt' [output_name] [denoise_strength]",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    output_name = sanitize_output_name(sys.argv[3] if len(sys.argv) > 3 else "", source_path)
    denoise_strength = float(sys.argv[4]) if len(sys.argv) > 4 else 0.55
    if denoise_strength < 0 or denoise_strength > 1:
        print("denoise_strength must be between 0.0 and 1.0", file=sys.stderr)
        sys.exit(1)

    prefix = f"openclaw_modify_{int(time.time())}"
    ...
    print("Submitting image modification request...")
    workflow = build_modify_workflow(uploaded_name, prompt, denoise_strength, prefix)
    result = comfyui_json_request(
        "/prompt",
        {
            "prompt": workflow,
            "client_id": f"openclaw-modify-{uuid.uuid4().hex}",
        },
    )
    ...
    if not wait_for_completion():
        print("Timeout waiting for image modification", file=sys.stderr)
        sys.exit(1)
    ...
    if not source_output_path or not os.path.exists(source_output_path):
        print("Could not locate modified image", file=sys.stderr)
        sys.exit(1)
